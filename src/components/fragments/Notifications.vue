               ...
               <v-list two-line class="pt-0">
                    ...
                    >
                        ...
                        <template v-for="(item, index) in items">
                            <!-- article link -->
                            <v-list-item :class="{'remove-v-item--active-style' : 'v-item--active v-list-item--active'}" :key="'A'+ index" :to="{ name: 'SingleArticle', params: { category : item.categoryId, slug: item.articleId  }}" v-scroll-to="'#comments'">
                                <!-- user avatar -->
                                <v-list-item-avatar>
                                    <!-- image -->
                                    <v-img
                                        :src="item.userAvatar"
                                        :lazy-src="item.userAvatar"
                                        alt="John"
                                ...
                                <v-list-item-content :title="item.text">
...
export default {
    ...
    methods: {
        ...
        allNotifications() {
            ...
            .where('ownerId', '==', this.$store.state.user.data.uid) // get uid from vuex store
            .orderBy('time', 'desc')
            .limit(8)
            .onSnapshot((snapshot) => {
        ...
        unseenAllNotifications() {
            db.collection('notifications')
            .where('ownerId', '==', this.$store.state.user.data.uid).get() // get uid from vuex store
            .then((snapshot) => {
        ...
        pushNotifications(change, num) { // get data from passing function
            ...
            notify.time = moment(change.doc.data().time).endOf('hour').fromNow() // format time
            ...
            if(notify.articleId) {
                this.getArticleByNotifyId(notify) // call a function and passing data
            }
        },
        // get user data
        ...
        // get article data
        getArticleByNotifyId(notify) {
            db.collection("articles").doc(notify.articleId).get()
            .then((querySnapshot) => {
                if(querySnapshot.exists) {
                    notify.articleId = querySnapshot.data().slug
                    // get category slug by id
                    db.collection("categories").doc(querySnapshot.data().categoryId).get()
                    .then((querySnapshot) => {
                        notify.categoryId = querySnapshot.data().slug
                    })
                }
            })
        }
