  ...
  <div>
    ...
    <v-container v-if="articles != ''">
      ...
      <div class="text-center m-4" v-if="articles.length != 0 && articles.length != totalArticles">
            <v-btn color="black white--text" elevation="0" :loading="loading" @click="loadArticles(9)"><v-icon class="mr-1">mdi-reload</v-icon> Load More</v-btn>
      </div>
    </v-container>
...
export default {
    ...
    methods: {
      // load article
      loadArticles(limit) { // get passing limit form function
          this.articles = [] // set articles array empty because all articles will be loaded in a new way
          let num = 9 + limit // multiplying limit
