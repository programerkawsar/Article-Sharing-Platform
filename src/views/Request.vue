<template>
    <div class="request">
        ...
        <div class="head-text text-center m-4 mb-0">
            <h3 class="font-weight-bold mb-1">Request</h3>
            <p class="m-0 text-muted font-weight-bold">Choose your articles for slide view and <br> make a request for showing your article on website slider</p>
        </div>
        <!-- Header text end -->
        <!-- Container Start -->
...
export default {
    ...
    created() {
      ...
      .where('requestShow', '==', true) // you can send a request once for an article
      ...
      .onSnapshot(snapshot => {
        snapshot.docChanges().forEach(change => {
          if(change.type === 'added') {
            ...
            this.articles.push(article) // push all article
      ...
      allCount.onSnapshot((snapshot) => {
          this.totalArticles = snapshot.size // set total number
      })
      
      // filtering requested articles
      db.collection('articles')
      .where('userId', '==', this.$store.state.user.data.uid) // get uid form vuex
      .where('published', '==', 1) // article must be published
      .where('requestShow', '==', false)
      .onSnapshot(snapshot => {
        snapshot.docChanges().forEach(change => {
          ...
          next.onSnapshot((snapshot) => {
              snapshot.docChanges().forEach(change => {
                if(change.type === 'added') {
                  ...
                  if(this.$store.state.user.loggedIn) { // check logged in
                    this.checkArticleSaveOrNot(article) // call a function
                  }
                }
              })
